...
Vercel 배포 시 Production/Preview/Development 환경별로 각각 등록해야 하며, `NEXT_PUBLIC_SITE_URL`은 프리뷰마다 도메인이 달라지므로 Vercel의 `VERCEL_URL` 시스템 환경변수를 조합해 동적으로 구성하는 방식도 고려한다.
...
- [ ] `src/middleware.ts`에 세션 갱신 로직 연결

### Phase 1 — 인증
- [ ] `login-form.tsx`를 `signInWithPassword` 실호출로 교체
- [ ] `signup-form.tsx`를 `signUp` 실호출로 교체
- [ ] `app/auth/callback/route.ts` 구현
- [ ] Supabase 대시보드 "Confirm email" 정책 결정
- [ ] `layout.tsx`에서 서버 유저 조회 후 `SiteHeader`에 전달
- [ ] `components/auth/user-nav.tsx` 구현 (로그인 상태 드롭다운 + 로그아웃)
- [ ] `onAuthStateChange` 구독 처리로 클라이언트-서버 상태 동기화
- [ ] 로그인/로그아웃/회원가입 전체 플로우 수동 검증

### Phase 2 — DB 스키마 + RLS
- [ ] `profiles` 테이블 생성 및 RLS 정책 적용
- [ ] `handle_new_user` 트리거 등록
- [ ] `todos` 테이블 생성 (컬럼/인덱스)
- [ ] `todos` RLS 정책 적용 (select 공개 / insert·update·delete 소유자)
- [ ] `set_updated_at` 트리거 등록
- [ ] `supabase gen types typescript`로 `src/types/database.types.ts` 생성
- [ ] Supabase 클라이언트에 제네릭 타입 연결

### Phase 3 — 캘린더 UI
- [ ] `date-fns` 설치
- [ ] `src/lib/date/calendar.ts` 작성 (월 매트릭스 계산 함수)
- [ ] `components/calendar/calendar-grid.tsx` 구현
- [ ] `components/calendar/calendar-cell.tsx` 구현
- [ ] `components/calendar/calendar-header.tsx` 구현 (이전/다음 달 네비게이션)
- [ ] `components/calendar/todo-pill.tsx` 구현
- [ ] `app/page.tsx`에서 월별 서버 조회 및 그리드 렌더링 연결

### Phase 4 — 드로어 + 폼 + 에디터
- [ ] Tiptap 패키지 설치
- [ ] `components/editor/extensions.ts` 구성 (이미지 제외 기본 서식)
- [ ] `components/editor/tiptap-editor.tsx`, `tiptap-toolbar.tsx` 구현
- [ ] `lib/validations/todo.ts` 작성
- [ ] `components/todo-drawer/todo-drawer.tsx` 구현 (create/edit 모드 분기)
- [ ] `components/todo-drawer/todo-form.tsx` 구현 (react-hook-form + Controller + Tiptap 바인딩)
- [ ] `app/actions/todos.ts` Server Actions 구현 (create/update/delete/getById)
- [ ] `components/todo-drawer/delete-todo-button.tsx` 구현 (소유자 전용 노출)
- [ ] Sheet 너비 조정(`sm:max-w-lg` 이상)

### Phase 5 — 이미지 업로드
- [ ] `todo-images` 버킷 생성 및 mime/size 제한 설정
- [ ] `storage.objects` RLS 정책 적용
- [ ] `components/editor/upload-image.ts` 구현
- [ ] Tiptap `handleDrop`/`handlePaste` 연결
- [ ] 업로드 중 로딩 placeholder 처리
- [ ] 업로드 실패 시 에러 토스트 처리
- [ ] `image_paths` 저장 로직 연결

### Phase 6 — 배포 (Vercel)
- [ ] Vercel 프로젝트 연결
- [ ] Production/Preview/Development 환경변수 등록
- [ ] Supabase Auth Site URL / Redirect URLs에 Vercel 도메인 등록
- [ ] `next build` 로컬 검증
- [ ] 프리뷰 배포에서 이메일 인증 플로우 점검
- [ ] 프리뷰 배포에서 이미지 업로드 플로우 점검
- [ ] 프로덕션 배포
